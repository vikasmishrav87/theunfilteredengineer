...
import { Globe2, MessageCircle, ArrowRight, ShieldCheck, Zap, Radio, Activity, Cpu, Server } from 'lucide-react';
...
function Interactive3DGlobeCanvas({ activeCity, onSelectHub }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hubCoords = [
        ...
        { name: 'New York', lat: 40.7128, lon: -74.0060, color: 0x25D366 },
        { name: 'London', lat: 51.5074, lon: -0.1278, color: 0xFFC72E },
        ...
        { name: 'Tokyo', lat: 35.6762, lon: 139.6503, color: 0x25D366 },
        { name: 'Sydney', lat: -33.8688, lon: 151.2093, color: 0xFF4D00 },
      ];
      ...
      hubCoords.forEach((hub) => {
        ...
        // Outer Pulsing Ring
        const ringGeo = new THREE.RingGeometry(0.45, 0.7, 16);
  ...
  return (
    <div 
      ref={mountRef} 
      className="w-full h-80 sm:h-96 cursor-grab active:cursor-grabbing relative flex items-center justify-center overflow-hidden"
      title="Click and drag to rotate the 3D Worldwide Mesh Globe"
    >
      <div className="absolute top-3 left-3 px-3 py-1 rounded-full bg-[#FAF7EE]/10 border border-[#FAF7EE]/20 text-[10px] font-mono text-[#FFC72E] flex items-center gap-1.5 backdrop-blur-md z-10">
        <Radio className="size-3 animate-pulse text-[#25D366]" />
        <span>3D ROTATING MESH • DRAG TO EXPLORE</span>
      </div>

      {!webGlSupported && (
        <div className="text-center text-[#FAF7EE]/60 font-mono text-xs">
  ...
  return (
    <section id="worldwide" className="relative py-16 sm:py-28 bg-[#141414] text-[#FAF7EE] border-b-2 border-[#141414]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10 sm:mb-12">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="sticker-pill px-3 py-1 bg-[#FF4D00] text-[#FAF7EE] text-[10px] sm:text-xs font-black uppercase shadow-[2px_2px_0_0_#FFC72E]">
                <Zap className="size-3.5 inline mr-1" />
                DISTRIBUTED DEPLOYMENT MESH
              </span>
              <span className="sticker-pill px-3 py-1 bg-[#25D366] text-[#141414] text-[10px] sm:text-xs font-black uppercase shadow-[2px_2px_0_0_#FAF7EE]">
                99.999% SLA UPTIME
              </span>
            </div>
            <h2 className="font-display text-4xl sm:text-6xl font-black uppercase tracking-tight text-[#FAF7EE]">
        ...
        {/* Global Telemetry Summary Bar */}
        <div className="mb-10 grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="rounded-2xl border-2 border-[#FAF7EE]/20 bg-[#FAF7EE]/5 p-4 backdrop-blur-md">
            <span className="text-[10px] font-mono text-[#FAF7EE]/60 block uppercase">Primary Engineering HQ</span>
            <span className="font-display text-lg sm:text-xl font-black text-[#FFC72E]">Mumbai NOC (8ms)</span>
          </div>
          <div className="rounded-2xl border-2 border-[#FAF7EE]/20 bg-[#FAF7EE]/5 p-4 backdrop-blur-md">
            <span className="text-[10px] font-mono text-[#FAF7EE]/60 block uppercase">Total Senior Engineers</span>
            <span className="font-display text-lg sm:text-xl font-black text-[#25D366]">1,000+ Specialists</span>
          </div>
          <div className="rounded-2xl border-2 border-[#FAF7EE]/20 bg-[#FAF7EE]/5 p-4 backdrop-blur-md">
            <span className="text-[10px] font-mono text-[#FAF7EE]/60 block uppercase">Global Hubs Connected</span>
            <span className="font-display text-lg sm:text-xl font-black text-[#FF4D00]">9 Key Tech Nodes</span>
          </div>
          <div className="rounded-2xl border-2 border-[#FAF7EE]/20 bg-[#FAF7EE]/5 p-4 backdrop-blur-md">
            <span className="text-[10px] font-mono text-[#FAF7EE]/60 block uppercase">Security SLA</span>
            <span className="font-display text-lg sm:text-xl font-black text-[#FAF7EE]">Zero Breach Record</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {GLOBAL_HUBS.map((hub) => {
            ...
            return (
              <TiltCard
                key={hub.id}
                className="rounded-3xl border-2 border-[#FAF7EE]/20 bg-[#FAF7EE]/5 p-6 sm:p-8 backdrop-blur-md shadow-[5px_5px_0_0_#FF4D00] transition-all hover:border-[#FFC72E] flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <div>
                      <span className="font-display text-xl sm:text-2xl font-black uppercase text-[#FFC72E] block">
                        {cityName}
                      </span>
                      {countryName && (
                        <span className="text-[11px] font-mono text-[#FAF7EE]/60 uppercase">
                          {countryName}
                        </span>
                      )}
                    </div>
                  ...
                  <p className="text-xs font-bold uppercase text-[#FAF7EE]/85 mb-4 line-clamp-2">
                    {specialtyRole}
                  </p>

                  <div className="space-y-2 border-t border-[#FAF7EE]/15 pt-4 text-xs font-mono">
                    <div className="flex justify-between">
                      <span className="text-[#FAF7EE]/50">NETWORK LATENCY:</span>
                      <span className="text-[#25D366] font-bold">{latencyPing}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[#FAF7EE]/50">ACTIVE CLIENTS / SQUADS:</span>
                      <span className="text-[#FF4D00] font-bold">{clientCount} SQUADS</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[#FAF7EE]/50">COVERAGE:</span>
                      <span className="text-[#FAF7EE] font-bold">24/7 Follow-the-Sun</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-[#FAF7EE]/15">
                  <a
                    ...
                    className="w-full py-3 rounded-full bg-[#FAF7EE] hover:bg-[#FF4D00] text-[#141414] hover:text-[#FAF7EE] font-display text-xs font-black uppercase flex items-center justify-center gap-1.5 transition-colors shadow-[3px_3px_0_0_#141414] active:scale-95"
                  >
        ...
        <div className="mt-14 sm:mt-16 text-center flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link
            to="/contact"
            className="sticker-pill px-8 py-4 sm:px-10 sm:py-5 text-sm sm:text-base bg-[#FF4D00] hover:bg-[#FFC72E] hover:text-[#141414] text-[#FAF7EE] shadow-[5px_5px_0_0_#FAF7EE] cursor-pointer flex items-center justify-center gap-2 font-black uppercase"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="sticker-pill px-6 py-4 text-sm bg-[#25D366] text-[#141414] shadow-[4px_4px_0_0_#FAF7EE] cursor-pointer flex items-center justify-center gap-2 font-black uppercase"
          >
